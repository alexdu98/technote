...
CREATE TABLE IF NOT EXISTS `technote`.`token` (
  ...
  INDEX `IDX_TOKEN_DATE_EXPIRATION` (`date_expiration` ASC),
  INDEX `IDX_TOKEN_ACTIF` (`actif` ASC),
...
CREATE TABLE IF NOT EXISTS `technote`.`technote` (
  ...
  INDEX `IDX_TECHNOTE_TITRE` (`titre` ASC),
  INDEX `IDX_TECHNOTE_DATE_CREATION` (`date_creation` ASC),
  INDEX `IDX_TECHNOTE_PUBLIE` (`publie` ASC),
  INDEX `IDX_TECHNOTE_VISIBLE` (`visible` ASC),
...
CREATE TABLE IF NOT EXISTS `technote`.`commentaire` (
  ...
  INDEX `IDX_COMMENTAIRE_DATE_CREATION` (`date_creation` ASC),
  INDEX `IDX_COMMENTAIRE_VISIBLE` (`visible` ASC),
...
CREATE TABLE IF NOT EXISTS `technote`.`question` (
  ...
  INDEX `IDX_QUESTION_DATE_QUESTION` (`date_question` ASC),
  INDEX `IDX_QUESTION_RESOLU` (`resolu` ASC),
  INDEX `IDX_QUESTION_VISIBLE` (`visible` ASC),
  INDEX `IDX_QUESTION_TITRE` (`titre` ASC),
...
CREATE TABLE IF NOT EXISTS `technote`.`reponse` (
  ...
  INDEX `FK_ID_AUTEUR_MEMBRE_idx` (`id_auteur` ASC),
  INDEX `IDX_REPONSE_DATE_REPONSE` (`date_reponse` ASC),
  INDEX `IDX_REPONSE_VISIBLE` (`visible` ASC),
...
CREATE TABLE IF NOT EXISTS `technote`.`action` (
  ...
  INDEX `IDX_ACTION_DATE_ACTION` (`date_action` ASC),
