...
-- -----------------------------------------------------
-- Table `technote`.`droit`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `technote`.`droit` (
  `type` ENUM('get', 'add', 'edit', 'drop') NOT NULL,
  `cible` VARCHAR(31) NOT NULL,
  `autoriser` TINYINT(1) NOT NULL,
  `id_groupe` INT UNSIGNED NULL,
  `id_membre` INT UNSIGNED NULL,
  PRIMARY KEY (`type`),
  INDEX `FK_DROIT_ID_GROUPE_idx` (`id_groupe` ASC),
  INDEX `FK_DROIT_ID_MEMBRE_idx` (`id_membre` ASC),
  CONSTRAINT `FK_DROIT_ID_GROUPE`
  FOREIGN KEY (`id_groupe`)
  REFERENCES `technote`.`groupe` (`id_groupe`)
    ON DELETE RESTRICT
    ON UPDATE CASCADE,
  CONSTRAINT `FK_DROIT_ID_MEMBRE`
  FOREIGN KEY (`id_membre`)
  REFERENCES `technote`.`membre` (`id_membre`)
    ON DELETE RESTRICT
    ON UPDATE CASCADE)
